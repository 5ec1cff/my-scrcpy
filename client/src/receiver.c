...
process_msg(struct device_msg *msg) {
    switch (msg->type) {
        ...
        case DEVICE_MSG_TYPE_IME_CURSOR_CHANGED: {
            ...
            struct device_msg* data = malloc(sizeof(struct device_msg));
            memcpy(data, msg, sizeof(struct device_msg));
            event.type = DEVICE_IME_EVENT_ID;
            event.user.code = USER_IME_EVENT_ID;
            event.user.data1 = data;
            event.user.data2 = 0;
            SDL_PushEvent(&event);
            LOGD("pushed event type=%d", msg->type);
            break;
        }
    }
}
...
process_msgs(const unsigned char *buf, size_t len) {
    ...
    for (;;) {
        ...
        LOGD("receive device msg type=%d", msg.type);
