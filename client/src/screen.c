...
struct point
screen_convert_frame_to_window_coords(struct screen *screen, int32_t x, int32_t y) {
    unsigned rotation = screen->rotation;
    assert(rotation < 4);
    ...
    nx = (int64_t) ((nx + screen->rect.x) * screen->rect.w / w);
    ny = (int64_t) ((ny + screen->rect.y) * screen->rect.h / h);
    ...
    result.x = (int64_t) (nx * ww / dw);
    result.y = (int64_t) (ny * wh / dh);
