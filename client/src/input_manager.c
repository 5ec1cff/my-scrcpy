...
convert_input_key(const SDL_KeyboardEvent *from, struct control_msg *to,
                  bool prefer_text, uint32_t repeat, bool is_ime, bool is_composing) {
    ...
    if (!convert_keycode(from->keysym.sym, &to->inject_keycode.keycode, mod,
                         prefer_text, is_ime, is_composing)) {
        return false;
    }
...
input_manager_process_key(struct input_manager *im,
    ...
    struct control_msg msg;
    if (convert_input_key(event, &msg, im->prefer_text, im->repeat, im->use_ime, im->is_composing)) {
        if (!controller_push_msg(controller, &msg)) {
...
static void
input_manager_process_ime(struct input_manager* im,
                          const SDL_UserEvent *event) {
    struct device_msg *data = event->data1;
    switch (data->type) {
        ...
        case DEVICE_MSG_TYPE_IME_CURSOR_CHANGED: {
            int32_t x, y;
            x = (int32_t) data->cursor.x;
            y = (int32_t) data->cursor.y;
            struct point coords = screen_convert_frame_to_window_coords(im->screen, x, y);
            LOGD("IME Rect: screen coords = %d, %d; window coords = %d, %d", x, y, coords.x, coords.y);
            SDL_Rect rect = {
                .x = coords.x,
                .y = coords.y,
                .w = 0,
...
static Uint32 
ime_composing_status_callback(Uint32 interval, void* params)
{
    SDL_TimerID *id = params;
    LOGD("ime composing status callback, timer id=%d", *id);
    static SDL_Event e = {
        .type = IME_COMPOSING_STATUS_CHANGED
    };
    SDL_PushEvent(&e);
    if (SDL_RemoveTimer(*id) == SDL_FALSE) 
        LOGD("failed to remove timer %d", *id);
    free(params);
    return 0;
}

static void
input_manager_process_text_editing(struct input_manager *im, const SDL_TextEditingEvent *event) {
    struct control_msg msg;
    msg.type = CONTROL_MSG_TYPE_IME_COMPOSING;
    msg.ime_composing.text = strdup(event->text);
    im->composing_length = strlen(event->text);
    if (im->composing_length == 0) {
        SDL_TimerID *id = calloc(1, sizeof(SDL_TimerID));
        SDL_TimerID timer = SDL_AddTimer(100, ime_composing_status_callback, id);
        *id = timer;
        LOGD("created timer %d for composing status callback", timer);
    } else {
        im->is_composing = true;
        LOGD("composing status changed, set to true");
    }
    if (!controller_push_msg(im->controller, &msg)) {
        free(msg.ime_composing.text);
        LOGW("Could not request 'composing text'");
    }
}
...
input_manager_handle_event(struct input_manager *im, SDL_Event *event) {
    switch (event->type) {
        ...
        case SDL_TEXTEDITING:
            if (im -> control && im->use_ime) {
                input_manager_process_text_editing(im, &event->edit);
            }
            return true;
        case DEVICE_IME_EVENT_ID:
            LOGD("%d", event->user.code);
            if (event->user.code == USER_IME_EVENT_ID) {
                LOGD("received user event");
                input_manager_process_ime(im, &event->user);
            }
            return true;
        case IME_COMPOSING_STATUS_CHANGED:
            if (im->composing_length == 0) {
                LOGD("composing status changed, set to false");
                im->is_composing = false;
            }
            return true;
    }
