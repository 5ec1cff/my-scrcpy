...
control_msg_serialize(const struct control_msg *msg, unsigned char *buf) {
    ...
    switch (msg->type) {
        case CONTROL_MSG_TYPE_INIT_CONTROL: {
            size_t len = 1;
            len += write_string(msg->control_init.version, 128, &buf[len]);
            buffer_write32be(&buf[len], msg->control_init.maxSize);
            len += 4;
            buffer_write32be(&buf[len], msg->control_init.lockedVideoOrientation);
            len += 4;
        ...
        case CONTROL_MSG_TYPE_IME_COMPOSING: {
            size_t len =
                write_string(msg->ime_composing.text,
                             CONTROL_MSG_INJECT_TEXT_MAX_LENGTH, &buf[1]);
            return 1 + len;
        }
...
#define LOG_CMSG(fmt, ...) LOGV("input: " fmt, ## __VA_ARGS__)
    switch (msg->type) {
        ...
        case CONTROL_MSG_TYPE_INJECT_TEXT:
            ...
            break;
        case CONTROL_MSG_TYPE_IME_COMPOSING:
            LOG_CMSG("composing text \"%s\"", msg->ime_composing.text);
            break;
        case CONTROL_MSG_TYPE_INJECT_TOUCH_EVENT: {
